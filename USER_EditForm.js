    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            if (targetUser == "junreyd" || targetUser == "litoa") {
                ...
                $("div #form-group3").show();

            } else {
                $("div.form-group1").hide();
                $("div.form-group2").hide();
                $("div.form-group3").hide();
            }
...
function uploadFileSP_v2(fileArray, fileCount) {
    ...
    getFile.done(function(buffer, status, xhr) {
        ...
        $.ajax({
            ...
            error: function(err) {
                alert("Succes, but files failed to save. Check your files with the same file name or fomat e.g .word, .png, .jpeg and try again.");
                window.location.assign('https://intranet.houseofit.com.au/Lists/IssueReport/USER_AllItems.aspx');
            }
        });
